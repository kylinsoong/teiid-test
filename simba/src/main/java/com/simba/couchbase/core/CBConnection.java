...
import com.simba.jdbc.common.ProductInfoHandler;
...
public class CBConnection extends DSIConnection {
    ...
    public CBConnection(CBEnvironment environment) throws ErrorException {
        ...
        this.m_productInfoHandler = new ProductInfoHandler("SimbaCouchbaseJDBCDriver.lic", "Simba Couchbase JDBC Driver", productVersion, CBJDBCDriver.class);
    ...
    public void connect(ConnSettingRequestMap requestMap) throws ErrorException, BadAuthException {
        ...
        LogUtilities.logFunctionEntrance(this.m_log, new Object[0]);
        CommonCoreUtils.logConnectionFunctionEntrance(this.m_log, requestMap, 1, 0, 4, 1004);
        
//        try {
//            this.m_productInfoHandler.validateLicense();    
//        } catch (Exception ex) {
//            ErrorException err = CBQueryUtils.buildRegularErrorMessage(CBJDBCMessageKey.LICENSE_FILE_READ_ERROR.name(), new String[] { ex.getMessage() });
//            err.initCause(ex);
//            throw err;    
//        }
        
        System.out.println("#############################");
        System.out.println("## Skip License Validation ##");
        System.out.println("#############################");
        System.out.println();
        
        this.m_settings = getConnectionSettings(requestMap);
        this.m_client = new CBClient(this.m_settings, this.m_log, getWarningListener());
        CBSMMetadata schemaMapMeta = new CBSMMetadata(prepareSchemaMap(this.m_settings), this.m_log);
